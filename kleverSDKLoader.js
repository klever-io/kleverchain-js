...
async function browserLoadKleverSDK() {
  ...
  if (file.headers.get("Content-Type") !== "application/wasm") {
    console.log(
      `Could not load KleverSDK, expected "application/wasm" type got ${file.headers.get(
        "Content-Type"
      )}`
    );
    throw `Could not load KleverSDK, expected "application/wasm" type got ${file.headers.get(
      "Content-Type"
    )}`;
  }
